...
def plotting1D(u, Nd, Na, IV=None, figsize = (6,6), savename=None):
    import matplotlib.pyplot as plt
    withsemilog=True
    #fig, ((ax, ax2), (ax3, ax4)) = plt.subplots(2,2, sharex=True)
    plt.clf(); plt.cla()
    IV = None
    if IV:
        fig = plt.figure(figsize=(2*figsize[0], 1*figsize[1]))
        ax00 = fig.add_subplot(321)
        ax01 = fig.add_subplot(323)
        ax02 = fig.add_subplot(325)
        ax1 = fig.add_subplot(122)
        axes = (ax00,ax01,ax02,ax1)
        #plot IV:
        if True:
            #V = list(IV[0])+[uu[-1]]
            #I = list(IV[1])+[uuJ[-1]]
            ...
            I = IV[1]
            ax1.plot(V, I/max(I), ".-")
            ax1.plot(V[-1], np.array(I[-1])/max(I), "o")
            ax1.grid()
            ax1.set_xlabel(f"Collector - Emitter Voltage (V)")
            ax1.set_ylabel("Collector Current (normalized)")
            #ax1.set_ylim([0,.05])
    ...
    for X in [xmin+(xmax-xmin)/2]:
        ...
        ax02.semilogy(xx, uun, "b", label="$n$")
        ax02.semilogy(xx, uup, "g", label="$p$" )
        ax02.semilogy(xx, uuNd, "b--", label="$N_D$")
        ax02.semilogy(xx, uuNa, "g--", label="$N_A$")
        #ax02.semilogy(xx, uurho, "k", label="Space charge")

    plt.rcParams['mathtext.fontset'] = 'dejavusans'

    ax02.set_xlabel('Position Coordinate $y \mathrm{(\mu m)}$')
    ax00.set_ylabel("Potential \n (V)")
    ax01.set_ylabel(f"Concentration \n ({N_norma}/"+"$\mathrm{cm^3)}$")
    ax02.set_ylabel(f"Concentration \n ({N_norma}/"+"$\mathrm{cm^3)}$")
    ax00.grid()
    ax01.grid()
    ax02.grid()
    #ax01.semilogy()
    ...
    #ax01.set_ylim([-.5, 1])
    ax01.set_ylim([-.1, .2])
    ax02.set_ylim(bottom=1e-3*min(uuNd))
    #fig.suptitle(f'ldkjds') # title celeho figure
    ...
    #myplot.myplot_2d(Nd, ax, shading="flat", savename="aa.png")
    #return2((xx, yy, zz), uu, uup, uun, uuNd, uuNa, uurho)
...
def plotting_J(u, fig=None, savename=None, numpoints_x=11, numpoints_y=11, subdomains_obj=None, subdomains_levels=None, aspect_equal=False, figsize = (6, 6)):
    ...
    print("myeval x, y pre Jn hotovo")
    Jx = Jpx+Jnx
    Jy = Jpy+Jny
...
def plotting_PsipnIV(u, fig=None, savename=None, subdomains_obj=None, subdomains_levels=None, figsize = (6, 6), aspect_equal=False):
